...
export default function MemberProfilePage() {
  ...
  if (!member) {
    return <p className="text-center py-8 text-muted-foreground">Profile not found.</p>
  }
    ...
    <div className="mx-auto py-10 w-fit">
      <Card>
        ...
        <CardContent className="space-y-4 text-sm text-gray-700">
          ...
          <div><strong>Class:</strong> {member.class}</div>
          <div><strong>Gender:</strong> {member.gender === "MALE" ? "Male" : "Female"}</div>
          <div><strong>Position:</strong> {member.position}</div>
          <div><strong>Joined At:</strong> {format(new Date(member.joined_at), "PPP")}</div>
